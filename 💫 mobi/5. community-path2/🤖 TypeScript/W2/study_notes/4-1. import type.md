...
```js
// 변환 후 JS 코드
// type import 부분은 다른 부수효과를 발생 시키지 않을 것으로 예쌍되어 삭제됨
let newType = // ~
```

- TS 코드가 컴파일 될 때, 인터페이스와 타입은 사라져서 [방출된 결과물에 존재하지 않는다](https://github.com/JeongwooHam/FE_Study_Logs/assets/123251211/7176de7c-26d1-47d6-95ca-d24ca150d97c).
...
- 이렇게 타입들이 지워졌을 때, 위의 코드와 같은 상황에서 컴파일러는 <code>{ sampleTypes, showTypes }</code> 이것들 중 어떤 것이 타입을 export한 것이고, 어떤 것이 아닌지 항상 정확하게 구분해내지는 못한다.
- 이때 webpack은 변환된 JS코드를 실행할 때에 존재하지 않는 것에 대한 import의 결과로 에러를 발생시킨다.
...
- TS의 <code>transpileModule</code> API, Babel을 통해 프로젝트의 모듈을 하나씩 컴파일하려 할 때에는 정 반대의 에러가 생기기도 한다.
...
- 또한 단일 파일 단위로 코드를 변환할 때에 re-export가 타입 정보만을 내보내는 경우 해당 구문을 삭제하도록 식별자를 명시할 수 있는 기능을 제공하였다.
  ...
  - <code>export type { T } from './mod'</code>를 사용하여 타입 T를 re-export 할 수 있었다.
...
> <code>isolatedModules</code>의 문제점

- 만약 <code>isolatedModules</code>를 사용하면서 re-exporting을 하려면 아래와 같은 방식을 사용해야 했다.
  ```ts
  ...
  - import 구문을 토애 가져온 타입과 동일한 이름을 가진 타입을 현재 모듈에서 재정의하려 하면, 이름 충돌로 컴파일 오류를 발생시켰다.
...
- 부수효과에 사용될 import를 명시적으로 만들고 싶을 때 유용하다.
